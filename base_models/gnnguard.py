...
class GNNGuard(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, dropout=0.5, lr=0.01, drop=False, weight_decay=5e-4, n_edge=1,with_relu=True,
        ...
        self.gc1 = GCNConv(nfeat, nhid, bias=True,).to(self.device)
        self.gc2 = GCNConv(nhid, nclass, bias=True, ).to(self.device)
    ...
    def forward(self, x, adj):
        ...
        # print(x.device, edge_index.device, adj_values.device)
    ...
    def fit(self, features, adj, labels, idx_train, idx_val=None, idx_test=None, train_iters=81, att_0=None,
            attention=True, model_name=None, initialize=True, verbose=False, normalize=False, patience=510, ):
        '''
        ...
        self.labels = labels
        # print(self.adj_norm,self.features,self.labels)
        if idx_val is None:
    ...
    def _train_with_early_stopping(self, labels, idx_train, idx_val, train_iters, patience, verbose):
        ...
        for i in range(train_iters):
            self.train()
            optimizer.zero_grad()
            # print(self.features, self.adj_norm)
            output = self.forward(self.features, self.adj_norm)
            loss_train = F.nll_loss(output[idx_train], labels[idx_train])
            loss_train.backward()
            optimizer.step()
